...
func NewInternalHandler(m *mux.Router) http.Handler {
	...
	m.Get(apirouter.Telemetry).Handler(trace.TraceRoute(telemetryHandler))
	m.Get(apirouter.GraphQL).Handler(trace.TraceRoute(handler(serveGraphQL)))
	m.Get(apirouter.SiteConfigurationFileGetLatest).Handler(trace.TraceRoute(handler(serveSiteConfigurationFileGetLatest)))
	m.Get(apirouter.CoreConfigurationFileGetLatest).Handler(trace.TraceRouter(handler(serveCoreConfigurationFileGetLatest)))
